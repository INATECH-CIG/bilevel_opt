...
#gens_df = gens_df.iloc[:2]

# 24 hours of demand first increasing and then decreasing
demand_df = pd.read_csv("inputs/demand.csv", index_col=0).iloc[:4]
demand_df["price"] = 100

big_w = 1000  # weight for duality gap objective
k_max = 2  # maximum multiplier for strategic bidding
opt_gen = 'gen_1'  # generator that is allowed to bid strategically
# %%
model = Model()
...
def duality_gap_part_2_rule(model):
    expr = nu_max * demand_df.loc[:, "volume"].values
    expr += sum(
...
objective = model.add_objective(
    expr=-(
        ...
        - big_w * (duality_gap_part_1_rule(model) 
                   - duality_gap_part_2_rule(model))
    )
)
...
demand_constr = model.add_constraints(d <= demand_df.loc[:, "volume"].values, name="d_max")
...
ramp_up_constr = model.add_constraints(ramp_up_rule, coords=[gens, time], name="ramp_up")

# %% ramp down constraint
def ramp_down_rule(model, i, t):
    if t == 0:
        return -g[i, t] <= max(gens_df.at[i, "g_0"] - gens_df.at[i, "r_down"], gens_df.at[i, "g_min"])
    else:
        return g[i, t - 1] - g[i, t] <= gens_df.at[i, "r_down"]

ramp_down_constr = model.add_constraints(ramp_down_rule, coords=[gens, time], name="ramp_down")
...
start_up_constr = model.add_constraints(start_up_rule, coords=[gens, time], name="start_up")
...
shut_down_constr = model.add_constraints(shut_down_rule, coords=[gens, time], name="shut_down")
# %%
# dual constraints
...
gen_dual_constr = model.add_constraints(gen_dual_rule, coords=[gens, time], name="gen_dual")
...
status_dual_constr = model.add_constraints(status_dual_rule, coords=[gens, time], name="status_dual")
...
demand_dual_constr = model.add_constraints(demand_dual_rule, coords=[time], name="demand_dual")
# %%
model.solve(solver_name="gurobi", NonConvex = 2, InfUnbdInfo=1)
